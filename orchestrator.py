...
def process_student_query(user_query: str, route: str, student_profile: dict = None) -> dict:
    """
    Central orchestration layer. Route classification now happens ONCE, in
    bot.py (via intent_router.classify_route), and is passed in here --
    this function no longer re-derives it, avoiding a duplicate LLM call
    per message.
    """
    if route == "REASONING_ENGINE":
        # Temporary structural placeholder until Phase 6 CP-SAT logic goes live.
        # student_profile is accepted now so the eventual solver call has
        # everything it needs without changing this function's signature again.
        return {
    ...
    if route == "SQL_GRAPH":
        ...
        if not course_codes:
            ...
            if not matches:
                return {
                    "response": ("⚠️ I couldn't identify a specific course from your question. "
