...
AWAITING_YEAR, AWAITING_SEMESTER, AWAITING_STREAM, AWAITING_FAILED = range(4)
...
async def handle_failed_courses(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = update.message.text.strip()
    failed_courses = [] if text.lower() == 'none' else [c.strip() for c in text.split(',')]
    ...
    try:
        # Pass the structured data directly to the new orchestrator logic
        result = process_reasoning_request(student, year, sem, stream_name, failed_courses)
